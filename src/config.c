...
void config_print_usage(const char *prog) {
    fprintf(stderr,
            ...
            "                          N in [2..16].\n"
            "  --drift-comp           Enable clock-drift compensation (receiver). Smoothly adjusts the\n"
            "                          sample rate to track the sender's clock over long sessions.\n"
            "                          Adds ~1.4 ms latency. Uses a Lanczos SRC with a PI controller.\n"
